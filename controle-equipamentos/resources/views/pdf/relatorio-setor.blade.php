...
<head>
    <meta charset="utf-8">
    <title>Relatório de Inventário — {{ $setor->nome }}</title>
    <style>
        body {
...
<body>
                    ...
                    <tr>
                        ...
                        <td>
                            @forelse($eq->monitores as $m)
                                <div>#{{ $m->numero }}: <span style="font-family: monospace;">{{ $m->serial }}</span></div>
                            @empty
    ...
    @else
        <p style="color: #64748b; font-style: italic;">Nenhum computador cadastrado para esta secretaria.</p>
    @endif

    <!-- Seção: Periféricos Avulsos -->
    <div class="section-title">2. Periféricos & Equipamentos de Apoio</div>
    @if($setor->perifericos->count() > 0)
        <table>
            <thead>
                <tr>
    ...
    @else
        <p style="color: #64748b; font-style: italic;">Nenhum periférico cadastrado para esta secretaria.</p>
    @endif

    <!-- Assinatura de Conferência -->
    <div class="signature-box">
        <div class="signature-line">
            Responsável pelo Levantamento TI / Conferência
        </div>
    </div>

    <div class="footer">
        Documento gerado automaticamente pelo Sistema de Inventário TI
    </div>
</body>
</html>
