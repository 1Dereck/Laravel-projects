...
<head>
    <meta charset="utf-8">
    <title>Relatório de Inventário — {{ $secretaria->secretaria }}</title>
    <style>
        body {
...
<body>
    <div class="header">
        ...
        <h2>Relatório da Diretoria / Divisão Hospitalar: {{ $secretaria->secretaria }}</h2>
        <div class="meta-info">
            <span><strong>Denominação Extensa:</strong> {{ $secretaria->nome_extenso }}</span> |
            <span><strong>Unidades / Setores Vinculados:</strong> {{ $secretaria->locais_atribuidos->count() }} unidade(s)</span>
        </div>
        <p>Departamento de Tecnologia da Informação & Comunicação | Gerado em: {{ $dataGeracao }}</p>
    </div>

    @forelse($secretaria->locais_atribuidos as $loc)
        <div class="local-block">
            <div class="local-title">
                LOCAL: {{ $loc->local }}
                @if($loc->bairro) <span style="font-weight: normal; font-size: 10px; color: #64748b;">(Bairro: {{ $loc->bairro }})</span> @endif
            </div>

            <!-- Computadores e Monitores -->
            <div class="section-title">Computadores & Monitores ({{ $loc->equipamentos->count() }})</div>
            @if($loc->equipamentos->count() > 0)
                <table>
                    <thead>
                        <tr>
                    ...
                    <tbody>
                        @foreach($loc->equipamentos as $eq)
                            <tr>
                                ...
                                <td>
                                    ...
                                    @empty
                                        <span style="color: #94a3b8; font-style: italic;">Sem monitores</span>
                                    @endforelse
                                </td>
                            </tr>
                        @endforeach
            ...
            @else
                <p style="color: #64748b; font-style: italic; font-size: 10px; margin: 4px 0 10px 0;">Nenhum computador cadastrado neste local.</p>
            @endif

            <!-- Periféricos Avulsos -->
            <div class="section-title">Periféricos & Apoio ({{ $loc->perifericos->count() }})</div>
            @if($loc->perifericos->count() > 0)
                <table>
                    <thead>
                        <tr>
                    ...
                    <tbody>
                        @foreach($loc->perifericos as $per)
                            <tr>
            ...
            @else
                <p style="color: #64748b; font-style: italic; font-size: 10px; margin: 4px 0 10px 0;">Nenhum periférico cadastrado neste local.</p>
            @endif
        </div>
    @empty
        <p style="color: #64748b; font-style: italic;">Nenhum local vinculado a esta secretaria.</p>
    @endforelse
