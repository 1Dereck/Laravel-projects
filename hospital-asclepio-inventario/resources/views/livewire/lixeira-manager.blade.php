<div class="space-y-6">
    ...
    <!-- Tabs Navigation (Responsive: 3 Grid columns on Mobile, classic horizontal tabs on Desktop) -->
    <!-- Mobile View (3 Grid columns) -->
    <div class="grid grid-cols-3 gap-2 sm:hidden bg-slate-100 dark:bg-slate-800/60 p-1.5 rounded-2xl border border-slate-200 dark:border-slate-800">
        <button wire:click="$set('activeTab', 'equipamentos')"
                class="py-2.5 px-2 text-xs font-bold rounded-xl transition-all flex items-center justify-center gap-1 cursor-pointer {{ $activeTab === 'equipamentos' ? 'bg-white dark:bg-slate-900 text-emerald-600 dark:text-emerald-400 shadow-sm border border-slate-200 dark:border-slate-700' : 'text-slate-600 dark:text-slate-400 hover:text-slate-900' }}">
        ...
        <button wire:click="$set('activeTab', 'perifericos')"
                class="py-2.5 px-2 text-xs font-bold rounded-xl transition-all flex items-center justify-center gap-1 cursor-pointer {{ $activeTab === 'perifericos' ? 'bg-white dark:bg-slate-900 text-emerald-600 dark:text-emerald-400 shadow-sm border border-slate-200 dark:border-slate-700' : 'text-slate-600 dark:text-slate-400 hover:text-slate-900' }}">
        ...
        <button wire:click="$set('activeTab', 'monitores')"
                class="py-2.5 px-2 text-xs font-bold rounded-xl transition-all flex items-center justify-center gap-1 cursor-pointer {{ $activeTab === 'monitores' ? 'bg-white dark:bg-slate-900 text-emerald-600 dark:text-emerald-400 shadow-sm border border-slate-200 dark:border-slate-700' : 'text-slate-600 dark:text-slate-400 hover:text-slate-900' }}">
            <span>Monitores</span>
                            ...
                            <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/40">
                                ...
                                <td class="px-6 py-4">
                                    <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 border border-indigo-500/20 shadow-xs">
                                        ...
                                        {{ $eq->local->local ?? $eq->setor->nome ?? 'N/A' }}
                                    </span>
                                </td>
                            ...
                            <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/40">
                                ...
                                <td class="px-6 py-4">
                                    <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 border border-indigo-500/20 shadow-xs">
                                        ...
                                        {{ $per->setor->nome ?? $per->local->local ?? 'N/A' }}
                                    </span>
                                </td>
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-800/80">
                        @forelse($trashedMonitores as $mon)
                            <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/40">
                                ...
                                <td class="px-6 py-4 font-mono text-emerald-600 dark:text-emerald-400">
                                    {{ $mon->serial }}
                                    @if($mon->marca_modelo)
                                        <span class="font-sans text-xs text-slate-500 font-normal">({{ $mon->marca_modelo }})</span>
                                    @endif
                                </td>
                ...
                @forelse($trashedMonitores as $mon)
                    <div class="p-5 space-y-3">
                        <div class="flex items-center justify-between gap-2">
                            ...
                            <span class="font-mono text-xs text-emerald-600 dark:text-emerald-400 font-bold">
                                {{ $mon->serial }} @if($mon->marca_modelo)<span class="font-sans text-slate-500 font-normal">({{ $mon->marca_modelo }})</span>@endif
                            </span>
                        </div>
